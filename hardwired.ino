...
const uint8_t L_BACK = D1;
const uint8_t L_FORW = D2;
const uint8_t L_PWM = D3;
const uint8_t Enc_A = D5;
const uint8_t Enc_B = D8;
...
void setupPins()
{
  ...
  attachInterrupt(digitalPinToInterrupt(Enc_A),detect,CHANGE);
  attachInterrupt(digitalPinToInterrupt(Enc_B),detect,CHANGE);
...
void setupWiFi()
{
  ...
  WiFi.disconnect();
  WiFi.mode(WIFI_STA);
  wifi.addAP("RoboticsClubIITBHU", "RoboConnect");
